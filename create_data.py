...
for root, dirs, files in os.walk("./sounds/"):
    for f in files:
        # load audio file
        data, sampling_rate = librosa.load("./sounds/"+f)
        # create figure, remove borders
        fig = plt.figure(n, figsize=(12, 4))
        n = n+1
        ax = fig.add_axes([0, 0, 1, 1])
        ax.axis('off')
        # plt.title(f)
        S = librosa.feature.melspectrogram(y=data, sr=sampling_rate)
        librosa.display.specshow(librosa.power_to_db(S, ref=np.max), y_axis='log', fmax=8000, x_axis='time')
